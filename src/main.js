...
function checkRotation() {
  ...
  containerScrollWidth =
    container.offsetWidth - window.innerWidth + (window.innerHeight * 1.15) / 3;
...
const scrollFunction = () => {
  if (mobileVersion) {
    ...
    if (window.scrollY > window.innerHeight * 0.05) {
      if (window.scrollY < (window.innerHeight * 1.15) / 3) {
        ...
        const opacityValue =
          1 -
          ((window.scrollY - window.innerHeight * 0.05) / window.innerHeight) *
            3;
        ...
        // background position
        if (
          introImg.getBoundingClientRect().bottom < window.innerHeight &&
          ...
          introImg.style.backgroundPositionX =
            (1 - introImg.getBoundingClientRect().bottom / window.innerHeight) *
              100 +
            '%';
        }
      }
  ...
  if (window.scrollY > window.innerHeight * 0.05) {
    if (window.scrollY < (window.innerHeight * 1.15) / 3) {
      ...
      const opacityValue =
        1 -
        ((window.scrollY - window.innerHeight * 0.05) / window.innerHeight) * 3;
      bigHeader.style.opacity = opacityValue;
      awards.style.opacity = opacityValue;
      ...
      if (window.scrollY < containerScrollWidth) {
        ...
        if (
          ...
          introImg.style.backgroundPositionX =
            (1 -
              (projectOffset + window.innerHeight * 0.684) /
                (window.innerWidth + window.innerHeight * 0.684)) *
              100 +
            '%';
        }
      } else {
      ...
      if (clubTitle.getBoundingClientRect().top < window.innerHeight) {
        fixedCharger.style.transform =
          'translateY(' +
          (clubTitle.getBoundingClientRect().top - window.innerHeight - 30) +
          'px)';
...
const motorControl = () => {
  ...
  if (motorSwitch.checked)
    ...
    if (matrix) {
      ...
      const a = values[0];
      const b = values[1];
      const matrixRotation = Math.round(Math.atan2(b, a) * (180 / Math.PI));
